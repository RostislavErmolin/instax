...
/* === Статистика ===
   Пишем события в collect.php. Ни кук, ни IP — только обезличенные счётчики.
   sendBeacon не теряет событие, когда посетитель тут же уходит по ссылке. */
const track = (event, extra = {}) => {
  if (location.protocol === "file:") return;          // при локальном открытии слать некуда
  try {
    const body = JSON.stringify({ event, ...extra });
    if (navigator.sendBeacon) navigator.sendBeacon("collect.php", body);
    else fetch("collect.php", { method: "POST", body, keepalive: true });
  } catch (e) { /* статистика не должна ломать страницу */ }
};

track("page_view", {
  ref: document.referrer ? new URL(document.referrer).hostname : "напрямую",
  width: window.innerWidth
});

document.querySelectorAll("[data-t]").forEach(el =>
  el.addEventListener("click", () => track("click", { place: el.dataset.t })));

document.querySelectorAll(".qa").forEach((el, i) =>
  el.addEventListener("toggle", () => { if (el.open) track("faq", { question: i + 1 }); }));

/* Состояние калькулятора шлём один раз через 3 секунды после последнего действия,
   иначе каждое нажатие «+» дало бы отдельную строку. */
let calcTimer = null;
const trackCalc = () => {
  clearTimeout(calcTimer);
  calcTimer = setTimeout(() => track("calc", {
    model: DATA[model].name + (model === 12 && two ? " x2" : ""),
    days,
    frames,
    total: priceFor(model, days) * (model === 12 && two ? 2 : 1) + (FRAMES[frames] || 0) + BOOKING
  }), 3000);
};
["#seg-model", "#seg-frames"].forEach(sel =>
  document.querySelectorAll(sel + " button").forEach(b => b.addEventListener("click", trackCalc)));
["f-minus", "f-plus", "f-own", "d-minus", "d-plus"].forEach(id =>
  $(id).addEventListener("click", trackCalc));
$("days").addEventListener("input", trackCalc);
$("frames").addEventListener("input", trackCalc);
